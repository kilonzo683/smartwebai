...
import { File, Trash2, FileQuestion, BookOpen, Loader2, FolderOpen } from "lucide-react";
...
interface DocumentLibraryProps {
  ...
  onCreateSummary: (document: Document) => void;
}

export function DocumentLibrary({ onGenerateQuiz, onCreateSummary }: DocumentLibraryProps) {
  ...
  if (isLoading) {
    return (
      <div className="glass rounded-2xl p-4 animate-slide-up" style={{ animationDelay: "400ms" }}>
        <h3 className="text-sm font-medium text-muted-foreground mb-3">Document Library</h3>
    ...
    <>
      <div className="glass rounded-2xl p-4 animate-slide-up" style={{ animationDelay: "400ms" }}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-medium text-muted-foreground">Document Library</h3>
        ...
        {documents.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground/70 mt-1">
              Upload lecture notes to get started
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Button
                    ...
                    title="Generate Quiz"
                  >
                    <FileQuestion className="w-4 h-4 text-agent-lecturer" />
                  </Button>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-7 w-7"
                    onClick={() => onCreateSummary(doc)}
                    title="Create Summary"
                  >
                    <BookOpen className="w-4 h-4 text-agent-lecturer" />
                  </Button>
                  <Button
                    variant="ghost"
                    size="icon"
